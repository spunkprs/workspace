...
public class TaxiAllotmentOne {
	...
	public static void main(final String ar[]) {
		...
		try {
			final List<Pair> users = new ArrayList<TaxiAllotmentOne.Pair>();
	...
	public static void fetchIndexesOfTaxis(final List<Pair> users, final int numberOfTaxis) {
	 ...
	 private static void parseUsersList(final List<Pair> users) {
		for (Pair p : users) {
			if (!taxiSet.isEmpty()) {
				System.out.print(taxiSet.first());
				System.out.print(" ");
				prepareTaxiNode(taxiSet.first(), p);
				taxiSet.remove(taxiSet.first());
			} else {
				System.out.print(parseTaxiNodesToFindAvailableTaxi(p));
				System.out.print(" ");
			}
		}
	}
